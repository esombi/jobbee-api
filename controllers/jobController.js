...
// Update a job => /api/v1/job/:1d
exports.updateJob = catchAsyncErrors (async (req, res, next) => {
    let job = await Job.findById(req.params.id);
 
    if(!job || job.length === null) {  
...
exports.getJob = catchAsyncErrors (async (req, res, next) => {
    const job = await Job.find({$and: [{_id: req.params.id}, {slug: req.
...
//Search jobs with radius => /api/v1/job/:zipcode/:distance
exports.getJobInRadius = catchAsyncErrors (async (req, res, next) =>{
    const { zipcode, distance } = req.params;

    //Getting longitude and latitude from geocoder with geocode
    const loc = await geoCoder .geocode(zipcode);
    const latitude = loc[0].latitude;
    const longitude = loc[0].longitude;

    const radius = distance/3963; // 3963 - earth's radius in miles
...
exports.jobStats = catchAsyncErrors (async (req, res, next) => {
    ...
    if(stats.length === 0){
        return next(new ErrorHandler(`no stats found for  ${req.params.topic}`,
        200))
    }
